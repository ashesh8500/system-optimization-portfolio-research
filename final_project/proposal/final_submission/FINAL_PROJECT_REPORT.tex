...
\section{Game-Theoretic Extension: Multi-Agent Equilibrium}
The single-agent layered control formulation in the preceding sections treats each screen/controller stack as an independent decision rule evaluated in isolation. In a real market, multiple strategy families may compete for the same candidate assets, and their combined allocations can create crowding effects that change the effective opportunity set. This section extends the layered formulation by modeling each strategy arm as a player in a non-cooperative game and computing Nash equilibrium allocations.

\subsection{Game formulation}
Let $\mathcal{A}=\{1,\dots,M\}$ be a set of strategy arms, each defined by a screen/controller pair from the earlier study. Each player $a\in\mathcal{A}$ chooses a portfolio weight vector $w_a\in\Delta_n$, where $\Delta_n=\{w\in\R^n:\mathbf{1}^\top w=1,\; w\ge0\}$ is the long-only simplex over a shared candidate universe of $n$ assets. Player $a$'s utility is:

\begin{equation}
\begin{aligned}
U_a(w_a,w_{-a})&=\alpha_a^\top w_a - \lambda w_a^\top\Sigma w_a - \kappa\|w_a-w_a^{\text{prev}}\|_1 \\
&\qquad - \gamma\, w_a^\top\!\Bigl(\sum_{j\neq a} w_j\Bigr),
\end{aligned}
\label{eq:player_utility}
\end{equation}
where $\alpha_a\in\R^n$ is player $a$'s expected-return proxy (derived from its screen signal), $\Sigma\succ0$ is the shared covariance matrix, $\lambda$ is the risk-aversion coefficient, $\kappa$ is the turnover cost, and $\gamma\ge0$ is the crowding sensitivity parameter. The final term penalizes allocation overlap with other players, modeling the adverse price pressure that results when multiple strategies hold the same positions simultaneously.

\subsection{Nash equilibrium and best-response dynamics}
A pure-strategy Nash equilibrium is a tuple $w^*=(w_1^*,\dots,w_M^*)$ such that for every player $a$:

\begin{equation}
U_a(w_a^*,w_{-a}^*)\ge U_a(w_a,w_{-a}^*)\quad\forall w_a\in\Delta_n.
\label{eq:nash_def}
\end{equation}

Holding other players' weights fixed, each player's best-response problem is convex:

\begin{equation}
\begin{aligned}
w_a^{\text{BR}}=\argmax_{w\in\Delta_n}\;&\alpha_a^\top w - \lambda w^\top\Sigma w - \kappa\|w-w_a^{\text{prev}}\|_1 \\
&- \gamma\, w^\top c_{-a},
\end{aligned}
\label{eq:best_response}
\end{equation}
where $c_{-a}=\sum_{j\neq a}w_j$ is the crowding vector. This is a convex quadratic program with an $\ell_1$ turnover penalty, solvable via standard interior-point methods. The equilibrium is computed via Gauss-Seidel best-response dynamics: initialize all players at the equal-weight portfolio; for each player $a$ in sequence, solve \eqref{eq:best_response} holding all other weights fixed; repeat until the maximum $\ell_1$ change across players falls below $\varepsilon=10^{-4}$.

Since the objective in \eqref{eq:player_utility} is strictly concave in $w_a$ (the risk term $-\lambda w_a^\top\Sigma w_a$ ensures this for $\lambda>0$), the strategy sets are convex and compact, and the interaction is linear, best-response dynamics are guaranteed to converge to a Nash equilibrium under diagonal dominance of the interaction matrix — that is, when $\gamma$ is sufficiently small relative to $\lambda$.

\subsection{Experiment A: Strategy families as players}
The game is instantiated with $M=6$ players drawn from the canonical strategy arms in the single-agent study, all operating on the top-500 candidate universe: 21-day momentum with equal weighting, 63-day momentum with equal weighting, 63-day low-volatility with equal weighting, 63-day volatility-adjusted momentum with rank-and-hold (21-day), 63-day momentum with rank-and-hold, and cluster-capped momentum with equal weighting. Equilibrium is computed at two representative dates (January 2020 and December 2024) with $\gamma=0.05$.

\begin{table}[!t]
\centering
\caption{Single-agent versus equilibrium utilities at the recent evaluation date ($\gamma=0.05$, $\lambda=1.0$, $\kappa=0.001$). Single-agent utility is computed with $\gamma=0$ (no crowding). Equilibrium utility includes the crowding penalty. Negative efficiency loss indicates improvement in equilibrium.}
\label{tab:equilibrium_utilities}
\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
Strategy arm & Single-agent & Equilibrium & Eff.\ loss & HHI \\
\midrule
21d mom / EW & 0.205 & 0.209 & $-$1.9\% & 0.102 \\
63d mom / EW & 0.187 & 0.185 & 1.1\% & 0.098 \\
63d low-vol / EW & $-$0.046 & $-$0.015 & +67.4\% & 0.057 \\
63d VAM / RH 21d & 0.039 & 0.048 & $-$23.1\% & 0.091 \\
63d mom / RH 21d & 0.625 & 0.614 & 1.8\% & 0.178 \\
Cluster mom / EW & 0.192 & 0.192 & 0.0\% & 0.101 \\
\bottomrule
\end{tabular}%
}
\end{table}

Table~\ref{tab:equilibrium_utilities} reports the per-player utility comparison. The efficiency loss is modest for most players (under 2\%), indicating that at moderate crowding levels, the equilibrium allocation closely approximates the independent single-agent optima. The low-volatility player shows a notable improvement in equilibrium because the crowding penalty discourages other players from holding the same low-volatility assets, effectively reducing the competition for those positions. The Herfindahl indices (HHI, ranging from 0.057 to 0.178) confirm that allocations remain reasonably diversified under equilibrium, with effective asset counts of 6--18 across players.

\subsection{Experiment B: Crowding sensitivity}
The crowding coefficient $\gamma$ is swept over $\{0,0.01,0.05,0.1,0.25,0.5,1.0\}$ on the recent evaluation date. Figure~\ref{fig:crowding_sweep} summarizes the trajectory.

\begin{figure}[!t]
\centering
\begin{tikzpicture}
\begin{axis}[
    width=0.85\columnwidth, height=0.42\columnwidth,
    xlabel={Crowding coefficient $\gamma$},
    ylabel={Aggregate overlap},
    grid=major, grid style={gray!20},
    tick label style={font=\footnotesize},
    label style={font=\small},
]
\addplot[mark=*, blue, thick] coordinates {
    (0.00, 0.246) (0.01, 0.141) (0.05, 0.103) (0.10, 0.067) (0.25, 0.024) (0.50, 0.000) (1.00, 0.000)
};
\end{axis}
\end{tikzpicture}
\caption{Aggregate pairwise portfolio overlap as a function of crowding sensitivity $\gamma$. At $\gamma=0$, players overlap on approximately 25\% of their allocation mass. As crowding increases, players diversify away from each other, and by $\gamma=0.5$ the equilibrium allocations are fully disjoint.}
\label{fig:crowding_sweep}
\end{figure}

Three regimes emerge. At $\gamma=0$, players act independently and overlap on approximately 25\% of their allocation mass — the natural consequence of multiple momentum and low-volatility screens selecting from the same candidate set. For $0<\gamma\le0.10$, overlap declines monotonically as the crowding penalty induces partial separation: players shift weight away from jointly held assets toward assets held only by themselves. At $\gamma\ge0.25$, the equilibrium collapses to near-zero overlap — each player occupies a nearly disjoint subset of the candidate universe. The total system utility remains stable across this transition, declining by only 3\% from $\gamma=0$ to $\gamma=1.0$, because the crowding penalty is a transfer rather than a pure loss: what one player loses from reduced position overlap, others gain from reduced competition.

\subsection{Experiment C: Temporal stability}
Equilibrium is recomputed at 20 quarterly-spaced rebalance windows spanning the full 2015--2024 sample. Convergence is achieved in every window (4--8 iterations per window), confirming that the best-response algorithm is numerically stable across market conditions.
...
\begin{axis}[
    width=0.85\columnwidth, height=0.42\columnwidth,
    xlabel={Window},
    ylabel={Utility share of dominant player},
    grid=major, grid style={gray!20},
    tick label style={font=\footnotesize},
    label style={font=\small},
    xtick={0,4,8,12,16,19},
    xticklabels={2015,2017,2019,2021,2023,2024},
]
\addplot[mark=*, blue, thick] coordinates {
    (0,0.32)(1,0.31)(2,0.34)(3,0.29)(4,0.33)(5,0.28)(6,0.35)(7,0.30)(8,0.33)(9,0.31)
    (10,0.36)(11,0.29)(12,0.34)(13,0.32)(14,0.33)(15,0.30)(16,0.35)(17,0.31)(18,0.34)(19,0.32)
};
\end{axis}
\end{tikzpicture}
\caption{Share of total equilibrium utility captured by the most successful player at each rebalance window. No single player dominates persistently; the utility distribution remains balanced across strategy families throughout the sample.}
\label{fig:temporal_stability}
\end{figure}

Figure~\ref{fig:temporal_stability} tracks the utility share of the dominant player at each window. No single strategy arm dominates persistently: the maximum share fluctuates between 0.28 and 0.36, and the composition of the leading player changes across windows. This temporal variation suggests that strategy dominance is not a structural property of any single screen/controller pair but depends on the prevailing market regime — consistent with the single-agent finding that the optimal stack changes with universe characteristics. The equilibrium framework provides a language for describing which strategies can coexist and when one displaces others.

The game-theoretic results complement the single-agent layered study. The single-agent analysis identifies which screen/controller stack performs best under independence. The equilibrium analysis asks a different question: if multiple strategies are deployed simultaneously and interact through crowding, which strategies sustain positive allocations and how does the collective outcome differ from the independent optimum? The answer, for this candidate universe and parameter range, is that moderate crowding ($\gamma\approx0.05$) induces relatively small efficiency losses, and no single strategy family eliminates the others — the equilibrium preserves a diversified ecology of allocation approaches.

\section{System Optimization Synthesis}
The project now spans two complementary perspectives on the same empirical substrate. The single-agent layered decomposition treats portfolio allocation as a sequential control problem with discrete universe/screen decisions, continuous weight optimization, and learned routing. The multi-agent equilibrium extension treats the same strategy families as players in a non-cooperative game with crowding externalities. Together, they illustrate a central methodological point: a system optimization problem can be productively studied from multiple vantage points, and the insights from each perspective are different.

The single-agent view answers: \textit{which control stack maximizes out-of-sample risk-adjusted performance for a solitary allocator?} The multi-agent view answers: \textit{which strategies survive when they compete, and how much efficiency is lost to crowding?}

The system optimization methods deployed across these perspectives form a coherent toolkit: convex quadratic programming for continuous weight selection, cardinality-constrained combinatorial optimization for screening, dynamic programming for sequential decision-making, policy-gradient reinforcement learning for adaptive routing, and game-theoretic best-response dynamics for multi-agent equilibrium. A system optimizer should select the appropriate tool based on whether the binding constraint is within a single controller (convex QP), across decision layers (bilevel optimization), or across interacting agents (game theory).
...
The game-theoretic extension in Section~IX demonstrates feasibility of the Nash equilibrium computation, but the current players share a single covariance matrix and a homogeneous crowding penalty. A richer model would give each player its own risk model and an asymmetric impact matrix calibrated to position sizes and liquidity. This would allow the crowding penalty to reflect realistic market microstructure: larger overlapping positions in less liquid names should generate larger crowding costs, and the equilibrium would then reveal whether certain strategy families are systematically crowded out of capacity-constrained assets.
...
The game-theoretic extension demonstrates that the crowding framework is more than a robustness check — it is a mechanism for constructing diversified strategy ensembles. When strategy families are treated as players in a non-cooperative game, the Nash equilibrium at moderate crowding ($\gamma\approx0.05$) induces partial separation while retaining approximately 97\% of independent utility. Strategy overlap drops from 25\% to 10\%, and no single strategy family eliminates the others — the equilibrium preserves a diversified ecology of allocation approaches. This suggests that a meta-allocator can use the equilibrium portfolio as a diversification tool: rather than selecting a single best strategy, deploy the equilibrium composition to reduce concentration risk while capturing the orthogonal alpha sources that different screens identify. The crowding sensitivity sweep shows a smooth transition from overlapping to disjoint allocations as $\gamma$ increases, confirming that $\gamma$ serves as a tunable parameter for engineering the desired level of strategy orthogonality. The temporal stability experiment further confirms that strategy dominance varies with market conditions rather than being structurally fixed, reinforcing the case for ensemble rather than singleton deployment. Together, the single-agent and multi-agent perspectives demonstrate that system optimization is not a single technique but a methodological stance: decompose the problem into interacting components, apply the appropriate optimization tool to each component, and evaluate the resulting system through rigorous out-of-sample validation.
...
\bibitem{fudenberg1991game} D.~Fudenberg and J.~Tirole, \emph{Game Theory}. MIT Press, 1991.
